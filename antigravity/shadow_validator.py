...
class ShadowValidator:
    ...
    async def shadow_heal_and_verify(
        self,
        healing_func,
        issue_type: str,
        issue: str,
        ...
        # Step 1: Create recovery point / 创建恢复点
        snapshot_id = await self._create_hot_snapshot()
        print(f"   ✅ Snapshot created: {snapshot_id}")
        
        try:
            # Step 2: Execute healing / 执行修复
            ...
            if not healing_result.get('success'):
                ...
                return {
                    ...
                    'reason': healing_result.get('message'),
                    'rolled_back': True
                }
            
            print(f"   ✅ Healing completed: {healing_result.get('message')}")
            
            # Step 3: Silent audit / 静默审计
            ...
            if new_score >= previous_score:
                ...
                return {
                    ...
                    'files_modified': healing_result.get('files_modified', []),
                    'rolled_back': False
                }
            else:
                # Failure - rollback
                ...
                return {
                    'status': 'ROLLED_BACK',
                    'reason': f'Repair did not improve score (delta: {improvement:.1f})',
                    'new_score': new_score,
                    'previous_score': previous_score,
                    'rolled_back': True
                }
        ...
        if (self.project_root / '.git').exists():
            try:
                ...
                result = subprocess.run(
                    ['git', 'stash', 'push', '-u', '-m', snapshot_id],
    ...
    async def _silent_audit(self) -> float:
        ...
        try:
            ...
            # Run static baseline audit only (fast)
