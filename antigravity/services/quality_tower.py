...
from antigravity.infrastructure.delivery_gate import DeliveryGate, DeliveryResult
...
def display_ceremonial_stamps(result: Optional[DeliveryResult]):
    ...
    with cols[0]:
        ...
        if result.local_signature and result.local_signature.signed:
            st.markdown(stamp_style.format(color='#00FF00', rotate='-5', opacity='1', text='✅ LOCAL SIGNED', subtext=f'Vibe Score: {result.local_signature.vibe_score:.1f}', signature=result.local_signature.signature), unsafe_allow_html=True)
            ...
            st.markdown(stamp_style.format(color='#888888', rotate='0', opacity='0.3', text='⏳ WAITING', subtext='Static/Dynamic Check', signature='Pending...'), unsafe_allow_html=True)
            st.caption('静态或动态审计未通过')
    with cols[1]:
        st.subheader('🌐 远程签名 (Remote)')
        if result.remote_signature and result.remote_signature.signed:
            st.markdown(stamp_style.format(color='#FFD700', rotate='5', opacity='1', text='✅ REMOTE SIGNED', subtext=f'Logic Score: {result.remote_signature.logic_score:.1f}', signature=result.remote_signature.signature), unsafe_allow_html=True)
            ...
            if result.local_signature and result.local_signature.signed:
                ...
                color = '#888888'
                opacity = '0.3'
            st.markdown(stamp_style.format(color=color, rotate='0', opacity=opacity, text=status_text, subtext=status_subtext, signature='Awaiting...'), unsafe_allow_html=True)
            if result.local_signature and result.local_signature.signed:
...
def display_trend_radar(project: Dict, history_manager: AuditHistoryManager):
    ...
    current = history[0]
    current_values = [current.get('vibe_score', 0), current.get('test_coverage', 0), current.get('logic_score', 0), max(0, 100 - current.get('security_issues', 0) * 10)]
    if len(history) > 1:
        previous = history[1]
        previous_values = [previous.get('vibe_score', 0), previous.get('test_coverage', 0), previous.get('logic_score', 0), max(0, 100 - previous.get('security_issues', 0) * 10)]
    else:
        previous_values = [0, 0, 0, 0]
    ideal_values = [100, 100, 100, 100]
    fig = go.Figure()
    fig.add_trace(go.Scatterpolar(r=ideal_values, theta=categories, fill='toself', fillcolor='rgba(0, 255, 0, 0.1)', line=dict(color='rgba(0, 255, 0, 0.3)', dash='dash', width=1), name='理想满分'))
    if len(history) > 1:
        fig.add_trace(go.Scatterpolar(r=previous_values, theta=categories, fill='toself', fillcolor='rgba(128, 128, 128, 0.2)', line=dict(color='rgba(128, 128, 128, 0.5)', width=2), name='上次审计'))
    fig.add_trace(go.Scatterpolar(r=current_values, theta=categories, fill='toself', fillcolor='rgba(0, 100, 255, 0.3)', line=dict(color='rgb(0, 100, 255)', width=3), name='当前审计'))
    fig.update_layout(polar=dict(radialaxis=dict(visible=True, range=[0, 100], tickfont=dict(size=10))), showlegend=True, height=450, margin=dict(l=80, r=80, t=40, b=40))
    ...
    for i, (metric, values) in enumerate(sparklines.items()):
        with spark_cols[i]:
            if values:
                fig_spark = go.Figure()
                fig_spark.add_trace(go.Scatter(y=values, mode='lines', line=dict(color='#0064FF', width=2), fill='tozeroy', fillcolor='rgba(0, 100, 255, 0.2)'))
                fig_spark.update_layout(showlegend=False, height=80, margin=dict(l=0, r=0, t=0, b=0), xaxis=dict(showticklabels=False, showgrid=False), yaxis=dict(showticklabels=False, showgrid=False, range=[0, 100]))
    ...
    for i, task in enumerate(recommended_tasks, 1):
        with st.expander(f'#{i} {task.healing_action} (ROI: {task.roi:.2f})', expanded=i == 1):
            ...
            if st.button(f'🔥 执行修复 (Execute Healing)', key=button_key, type='primary' if i == 1 else 'secondary'):
                show_remedy_preview(issue_type=task.issue_type, issue=task.issue, project=project)
...
def get_latest_audit_result(project: Dict) -> Optional[DeliveryResult]:
    ...
    """
    if 'latest_audit' in st.session_state and 'latest_audit_project' in st.session_state and (st.session_state['latest_audit_project'] == project['name']):
