...
import json
from .mission_orchestrator import MissionOrchestrator, AtomicTask, TaskState
from .local_reasoning import LocalReasoningEngine
from antigravity.services.sheriff_strategist import SheriffStrategist, OptimizedPlan
...
class AutonomousAuditor:
    ...
    def __init__(self, project_root: Optional[Path]=None):
        ...
        from antigravity.infrastructure.file_lock_manager import FileLockManager
    ...
    async def autonomous_run(self, idea: str) -> Dict:
        ...
        try:
            ...
            async with AsyncPipeline() as pipeline:
                review_future = pipeline.submit(self.remote_strategist.tune_plan, plan)
                for i, task in enumerate(tasks):
                    next_task = tasks[i + 1] if i + 1 < len(tasks) else None
                    result = await self._execute_task_with_pipeline(task, next_task, pipeline)
                    if not result['success']:
                        self._log('Task failed', {'task_id': task.task_id})
                        break
                optimized_plan = await pipeline.wait_for(review_future, timeout=30)
                if optimized_plan:
                    self._log('Plan optimized', {'score': optimized_plan.quality_score, 'approved': optimized_plan.approved})
    ...
    async def _execute_task_with_pipeline(self, task: AtomicTask, next_task: Optional[AtomicTask], pipeline: AsyncPipeline) -> Dict:
        """
        ...
        if next_task:
            ...
            next_review_future = pipeline.submit(self.remote_strategist.tune_plan, next_plan)
    ...
    def _generate_task_plan(self, task: AtomicTask) -> Dict:
        ...
        Returns:
            Task plan / 任务计划
        """
        return {'intent': task.goal, 'files_to_create': task.files_affected, 'dependencies': task.dependencies, 'tasks': [task.goal]}
    ...
    def _execute_task_logic(self, task: AtomicTask) -> Dict:
        ...
        return {'created_files': created_files, 'success': True}
    ...
    def _final_delivery_check(self) -> Dict:
        ...
        if summary['completion_rate'] < 1.0:
            return {'delivered': False, 'reason': 'incomplete_tasks', 'summary': summary}
        return {'delivered': True, 'summary': summary, 'timestamp': datetime.now().isoformat()}
    ...
    def _log(self, event: str, data: Optional[Dict]=None):
        ...
        """
        log_entry = {'timestamp': datetime.now().isoformat(), 'event': event, 'data': data or {}}
    ...
    def save_state(self, filepath: str):
        ...
        """
        state = {'idea': self.current_idea, 'execution_log': self.execution_log, 'orchestrator_state': self.orchestrator.get_execution_summary(), 'timestamp': datetime.now().isoformat()}
